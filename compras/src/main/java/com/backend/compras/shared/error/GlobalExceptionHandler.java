...
import com.backend.compras.shared.resiliencia.CircuitoAbiertoException;
...
public class GlobalExceptionHandler {
    ...
    ProblemDetail accesoDenegado(AuthorizationDeniedException ex) {
        metricas.accesoDenegado("compras");
    ...
    /** El destino no respondió o el cortacircuitos rechazó sin llamar. */
    @ExceptionHandler({ ServicioNoDisponibleException.class, CircuitoAbiertoException.class })
    ProblemDetail servicioCaido(RuntimeException ex) {
        return construir(HttpStatus.SERVICE_UNAVAILABLE, "Servicio no disponible", ex.getMessage());
    }

    /** Transición de estado no permitida o carrito vacío. */
    @ExceptionHandler(IllegalStateException.class)
    ProblemDetail estadoInvalido(IllegalStateException ex) {
        return construir(HttpStatus.CONFLICT, "Operación no válida", ex.getMessage());
    }

    /** Dos operaciones sobre la misma saga o el mismo producto a la vez. */
    @ExceptionHandler(ObjectOptimisticLockingFailureException.class)
    ProblemDetail conflictoConcurrencia(ObjectOptimisticLockingFailureException ex) {
        log.warn("Conflicto de concurrencia: {}", ex.getMessage());
        return construir(HttpStatus.CONFLICT, "Conflicto",
                "Otra operación modificó estos datos al mismo tiempo. Vuelve a intentarlo.");
    }
