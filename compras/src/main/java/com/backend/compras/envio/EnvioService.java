    ...
    public Envio crearParaPedido(Pedido pedido, SagaCheckout saga) {
        return repositorio.findByPedidoId(pedido.getId()).orElseGet(() -> {
            Envio envio = Envio.builder()
                    ...
                    .calle(saga.getCalle())
                    .numero(saga.getNumero())
                    .codigoPostal(saga.getCodigoPostal())
                    .distrito(saga.getDistrito())
                    .provincia(saga.getProvincia())
                    .departamento(saga.getDepartamento())
                    .pais(saga.getPais())
                    .receptorNombre(saga.getReceptorNombre())
